...
exports.yourPastWeek = async (req, res, next) => {
  try {
    const { user, query } = req;
    const response = await quizPollService.yourPastWeek(user, query);
    if (!response.success) {
      return responseHelper.errorResponse(
        res,
        response.code,
...
exports.getListArchives = async (req, res, next) => {
  try {
    const { user, query } = req;
    const response = await quizPollService.getListArchives(user.id, query);
    if (!response.success) {
      return responseHelper.errorResponse(
        res,
        response.code,
...
exports.addArchivesRecord = async (req, res, next) => {
  try {
    const { id } = req.params;
    const response = await quizPollService.addArchivesRecord(id);
    if (!response.success) {
      return responseHelper.errorResponse(
        res,
        response.code,
...
exports.bulkQuizPollActivated = async (req, res, next) => {
  try {
    const { body } = req;
    const response = await quizPollService.bulkQuizPollActivated(body);
    if (!response.success) {
      return responseHelper.errorResponse(
        res,
        response.code,
